...
import Somever from '@hapi/somever';
...
class Server {
    ...
    _clone(name) {
        ...
        return new Server(this._core, name, this);
    }
    ...
    control(server) {
        ...
        Hoek.assert(server instanceof Server, 'Can only control Server objects');
    ...
    decorate(type, property, method, options = {}) {
        ...
        if (type === 'handler') {
            ...
            if (typeof property === 'string') {
                Hoek.assert(Object.getOwnPropertyNames(Server.prototype).indexOf(property) === -1, 'Cannot override the built-in server interface method:', propertyName);
            }
            else {
                Hoek.assert(Object.getOwnPropertySymbols(Server.prototype).indexOf(property) === -1, 'Cannot override the built-in server interface method:', propertyName);
            }
